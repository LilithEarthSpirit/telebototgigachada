...
export const translateWords = async (text, lang = 'en|ru') => {
  ...
  const regex = /[.,\/#!$%\^&\*;:{}=\-_`~()]/g
  return {
    source: capitalize(data.matches[0].segment.replace(regex, '')),
    target: capitalize(data.responseData.translatedText.replace(regex, '')),
  }
}
...
export const getRandom = async (key) => {
  const result = []
  const words = randomWords(key)
